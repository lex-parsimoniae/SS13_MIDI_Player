...
intro_tab <- tabItem(
  ...
  box(width = 12,
      ...
      tags$p("This R Shiny web application takes as input uploaded MIDI files and outputs Python scripts that simulate keypresses on virtual instruments (1st tab), or to code that can be used with the Player Pianos or Text-to-Music MechComp components (2nd tab). This app was designed with the Goonstation branch of SS13 in mind, 
...
convert_tab_player <- tabItem(
  ...
  box(width = 12,
      collapsible = FALSE,
      title = "Convert midi to Player Piano or T2M Component input",
      tags$p('Takes a MIDI file and outputs a .csv file where the rows represent MIDI tracks (a song that exceeds the Player Piano or T2M Component character limit will need to be input as separate signals or additional pianos to play the entire song). Supports concurrent note playing, so each additional Player Piano/T2M Component will need to be set up to play sequentially, not simultaneously. Does not require any additional software.'),
      tags$br(),
      tags$b("NOTE: Percussion tracks and other tracks without notes do not play well with the converter, so make sure to remove those from the MIDI before trying to convert! If you wish to convert a MIDI with multiple tracks to a single instrument (e.g., a Player Piano), you will need to merge the tracks in the MIDI prior to conversion."), 
      tags$br(),
      tags$br(),
